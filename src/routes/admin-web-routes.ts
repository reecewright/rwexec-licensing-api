...
router.use((req, res, next) => {
  ...
  res.redirect = ((statusOrPath: number | string, maybePath?: string) => {
...
async function uniqueLicenseKey(productSlug: string) {
  for (let i = 0; i < 5; i += 1) {
    ...
    if (!(await prisma.license.findUnique({ where: { keyHash } })))
    ...
    if (!email)
      return res
        ...
        .send(
          layout(
            "Customer",
            `<div class="alert error">Account email is required.</div>`,
            "customers",
          ),
        );
    if (billingEmail && !/^\S+@\S+\.\S+$/.test(billingEmail))
      return res
        ...
        .send(
          layout(
            "Customer",
            `<div class="alert error">Billing email is not valid.</div>`,
            "customers",
          ),
        );
    ...
    if (existing)
      return res
        ...
        .send(
          layout(
            "Customer",
            `<div class="alert error">That account email already belongs to another customer.</div>`,
            "customers",
          ),
        );
    await prisma.customer.update({
      where: { id: req.params.id },
      data: { name, email, billingEmail },
    });
    res.redirect(
      `/admin/customers/${encodeURIComponent(req.params.id)}?saved=1`,
    );
  } catch (error) {
    next(error);
  }
    ...
    if (!customerEmailConfigured())
      return res
        ...
        .send(
          layout(
            "Customer portal",
            `<div class="alert error">Customer email delivery is not configured.</div>`,
            "customers",
          ),
        );
    await sendCustomerPortalEmail(req.params.id, "login");
    await writeAudit({
      action: "customer.portal_email_admin",
      entityType: "customer",
      entityId: req.params.id,
      summary: "Customer portal link emailed by admin",
    });
    res.redirect(
      `/admin/customers/${encodeURIComponent(req.params.id)}?saved=1`,
    );
...
router.get("/products", async (_req, res, next) => {
  try {
    const products = await prisma.product.findMany({
      ...
      include: {
        _count: {
          select: { plans: true, licenses: true, subscriptions: true },
        },
      },
    });
    ...
    if (!name || !slug)
      return res
        ...
        .send(
          layout(
            "Products",
            `<div class="alert error">Name and slug are required.</div>`,
            "products",
          ),
        );
...
router.post("/plans", async (req, res, next) => {
  try {
    const lines = String(req.body.entitlements ?? "")
      .split(/\r?\n/)
      .map((v) => v.trim())
      .filter(Boolean);
    const entitlements = lines
      .map((line) => {
        const [key, label, typeRaw, valueRaw] = line
          .split("|")
          .map((v) => v?.trim());
        const type =
          typeRaw?.toLowerCase() === "limit"
            ? ("LIMIT" as const)
            : ("BOOLEAN" as const);
        return {
          key,
          ...
          type,
          enabled:
            type === "BOOLEAN" ? valueRaw?.toLowerCase() !== "false" : null,
          limit:
            type === "LIMIT"
              ? Math.max(0, Number.parseInt(valueRaw || "0", 10) || 0)
              : null,
        };
      })
      .filter((e) => e.key);
    ...
    await prisma.plan.create({
      data: {
        ...
        billingInterval: String(req.body.billing_interval ?? "").trim() || null,
        stripeProductId:
          String(req.body.stripe_product_id ?? "").trim() || null,
        stripePriceId: String(req.body.stripe_price_id ?? "").trim() || null,
...
router.get("/subscriptions", async (_req, res, next) => {
  try {
    const [subscriptions, products, plans] = await Promise.all([
      prisma.subscription.findMany({
        orderBy: { createdAt: "desc" },
        include: { customer: true, product: true, plan: true, usage: true },
      }),
      prisma.product.findMany({
        where: { active: true },
        orderBy: { name: "asc" },
...
router.post("/subscriptions", async (req, res, next) => {
  try {
    ...
    const customer = await prisma.customer.upsert({
      where: { email },
      update: {
        name: String(req.body.customer_name ?? "").trim() || undefined,
        ...(billingEmail ? { billingEmail } : {}),
      },
      create: {
        email,
        name: String(req.body.customer_name ?? "").trim() || null,
        billingEmail,
      },
    });
    ...
    const subscription = await prisma.subscription.create({
      data: {
        ...
        productId: String(req.body.product_id),
        planId: String(req.body.plan_id ?? "") || null,
        status: complimentary ? "COMPLIMENTARY" : "ACTIVE",
        complimentary,
        currentPeriodEnd: req.body.current_period_end
          ? new Date(`${req.body.current_period_end}T23:59:59.000Z`)
          : null,
      },
    });
    await writeAudit({
      ...
      summary: `Manual subscription created for ${customer.name || customer.email}`,
    });
    res.redirect(
      `/admin/subscriptions/${encodeURIComponent(subscription.id)}?created=1`,
    );
  } catch (error) {
    next(error);
  }
  ...
  try {
    ...
    const subscriptionOptions = subscriptions
      .map((subscription) => {
        ...
        const billing = subscription.complimentary
          ? "Complimentary"
    ...
    const message =
      req.query.created === "1"
        ? `<div class="alert success">Licence created. The full key was shown once on the previous page.</div>`
        : "";
    const noSubscriptions =
      subscriptions.length === 0
        ? `<div class="alert error">Create an active or complimentary customer subscription before generating a licence.</div>`
        : "";
...
router.post("/licenses", async (req, res, next) => {
  try {
    ...
    if (!subscriptionId) {
      return res
        ...
        .send(
          layout(
            "Licences",
            `<div class="alert error">Select a customer subscription before generating a licence.</div>`,
            "licenses",
          ),
        );
    }
    ...
    if (
      ...
      return res
        ...
        .send(
          layout(
            "Licences",
            `<div class="alert error">That subscription is unavailable or is not currently entitled to a licence.</div>`,
            "licenses",
          ),
        );
    }
    ...
    const activationLimit = Math.max(
      ...
      Math.min(
        1000,
        Number.parseInt(String(req.body.activation_limit ?? "1"), 10) || 1,
      ),
    );
    const licence = await prisma.license.create({
      data: {
        ...
        activationLimit,
        expiresAt: expiresRaw ? new Date(`${expiresRaw}T23:59:59.000Z`) : null,
      },
    });
    ...
    await writeAudit({
      ...
      entityType: "license",
      entityId: licence.id,
      summary: `Licence created for ${customer.name || customer.email}`,
      metadata: { subscriptionId: subscription.id, activationLimit },
    });
    const planName = subscription.plan?.name || "Custom";
    const subscriptionLabel = subscription.complimentary
      ? "Complimentary"
      : subscription.status.replaceAll("_", " ");
...
router.get("/plans/:id", async (req, res, next) => {
  try {
    ...
    const entitlementText = plan.entitlements
      .map(
        (e) =>
          `${e.key}|${e.label}|${e.type === "LIMIT" ? "limit" : "boolean"}|${e.type === "LIMIT" ? (e.limit ?? 0) : e.enabled ? "true" : "false"}`,
      )
      .join("\n");
    const message =
      req.query.saved === "1"
    ...
    await prisma.$transaction(async (tx) => {
      await tx.plan.update({
        ...
        data: {
          ...
          priceMinor: price ? Math.round(Number(price) * 100) : null,
          billingInterval:
            String(req.body.billing_interval ?? "").trim() || null,
          stripeProductId:
            String(req.body.stripe_product_id ?? "").trim() || null,
          stripePriceId: String(req.body.stripe_price_id ?? "").trim() || null,
      ...
      if (entitlements.length)
        await tx.planEntitlement.createMany({
          data: entitlements.map((e) => ({ ...e, planId: req.params.id })),
        });
    });
    await writeAudit({
      action: "plan.updated",
      entityType: "plan",
      entityId: req.params.id,
      summary: "Plan settings, Stripe mapping and entitlements updated",
      metadata: {
        stripeProductId:
          String(req.body.stripe_product_id ?? "").trim() || null,
        stripePriceId: String(req.body.stripe_price_id ?? "").trim() || null,
  ...
  try {
    ...
    const body = `
      ...
      <section class="panel">
        ...
        <p class="muted">
          Manage the subscription lifecycle. Each action requires confirmation before it is applied.
        </p>

        <div class="actions">
          <a
            class="button secondary"
            href="/admin/subscriptions/${escapeHtml(subscription.id)}/confirm/reactivate"
          >
            Reactivate
          </a>
          ...
          <a
            ...
            href="/admin/subscriptions/${escapeHtml(subscription.id)}/confirm/suspend"
          >
            Suspend
          </a>
          ...
          ${
            subscription.currentPeriodEnd
              ? `
                <a
                  class="button secondary"
                  href="/admin/subscriptions/${escapeHtml(subscription.id)}/confirm/cancel_period_end"
                >
          ...
          <a
            class="button danger"
            href="/admin/subscriptions/${escapeHtml(subscription.id)}/confirm/cancel_now"
          >
            Cancel now
          </a>
          ...
          ${
            canDelete
              ? `
                <a
                  class="button danger"
                  href="/admin/subscriptions/${escapeHtml(subscription.id)}/confirm/delete"
                >
                  Delete permanently
                </a>
              `
...
router.post("/subscriptions/:id/update", async (req, res, next) => {
  try {
    ...
    if (planId) {
      ...
      if (!plan || plan.productId !== subscription.productId)
        return res
          .status(400)
          .send(
            layout(
              "Subscription",
              `<div class="alert error">That plan does not belong to this product.</div>`,
              "subscriptions",
            ),
          );
    }
    ...
    await prisma.subscription.update({
      ...
      data: {
        planId,
        complimentary,
        currentPeriodEnd: periodRaw
          ? new Date(`${periodRaw}T23:59:59.000Z`)
          : null,
        ...(complimentary &&
        ["ACTIVE", "COMPLIMENTARY"].includes(subscription.status)
          ? { status: "COMPLIMENTARY" }
          : !complimentary && subscription.status === "COMPLIMENTARY"
            ? { status: "ACTIVE" }
            : {}),
      },
    });
    await writeAudit({
      ...
      entityType: "subscription",
      entityId: subscription.id,
      summary: "Subscription details updated",
      metadata: { planId, complimentary, currentPeriodEnd: periodRaw || null },
    });
    res.redirect(
      `/admin/subscriptions/${encodeURIComponent(subscription.id)}?saved=1`,
    );
  } catch (error) {
    next(error);
  }
...
router.get("/subscriptions/:id/confirm/:action", async (req, res, next) => {
  try {
    const subscription = await prisma.subscription.findUnique({
      where: { id: req.params.id },
      include: {
    ...
    const confirmations: Record<
      string,
      { title: string; message: string; button: string; className: string }
    > = {
      reactivate: {
        title: "Reactivate subscription?",
        message:
          "This subscription will become active again and its linked licence will be allowed to validate.",
      ...
      suspend: {
        ...
        message:
          "This subscription will be suspended and its linked licence will stop validating until the subscription is reactivated.",
      ...
      cancel_period_end: {
        ...
        message:
          "This subscription will remain active until the current billing period ends, then cancellation will be handled at period end.",
        button: "Confirm period-end cancellation",
        className: "warning",
      },
      cancel_now: {
        title: "Cancel subscription now?",
        message:
          "This subscription will be cancelled immediately and its linked licence will no longer validate while the subscription is inactive.",
        button: "Confirm immediate cancellation",
        className: "danger",
      },
      delete: {
        title: "Permanently delete subscription?",
        message:
          "This permanently deletes the subscription and all linked licences. Activation history and stored licence delivery records for those licences will also be deleted. This cannot be undone.",
    ...
    if (!confirmation) {
      return res
        ...
        .send(
          layout(
            "Subscription",
            `<div class="alert error">Unknown subscription action.</div>`,
            "subscriptions",
          ),
        );
    }
    ...
    if (
      ...
      return res.status(400).send(
        layout(
          ...
          `<div class="alert error">This subscription must be canceled, expired or suspended before it can be permanently deleted.</div>
           <a class="button secondary" href="/admin/subscriptions/${escapeHtml(subscription.id)}">Back to subscription</a>`,
          "subscriptions",
        ),
      );
    }

    if (action === "cancel_period_end" && !subscription.currentPeriodEnd) {
      return res.status(400).send(
        layout(
          "Cannot cancel at period end",
          `<div class="alert error">This subscription does not have a current period end date.</div>
           <a class="button secondary" href="/admin/subscriptions/${escapeHtml(subscription.id)}">Back to subscription</a>`,
          "subscriptions",
        ),
      );
    ...
    const formAction =
      action === "delete"
        ? `/admin/subscriptions/${escapeHtml(subscription.id)}/delete`
        : `/admin/subscriptions/${escapeHtml(subscription.id)}/action`;
    ...
    const body = `
      <section class="panel">
        <h2>${escapeHtml(confirmation.title)}</h2>
        ...
        <dl class="detail-list">
          ...
          <div>
            <dt>Product</dt>
            <dd>${escapeHtml(subscription.product.name)}</dd>
          </div>
          <div>
            <dt>Plan</dt>
          ...
          <div>
            ...
            <dd>${escapeHtml(statusLabel(subscription.status))}</dd>
          </div>
        </dl>
        ...
        <div class="actions">
          <a
            class="button secondary"
            href="/admin/subscriptions/${escapeHtml(subscription.id)}"
          >
            Go back
          </a>
          ...
          <form method="post" action="${formAction}">
            ${hiddenAction}
            <button class="button ${confirmation.className}" type="submit">
              ${escapeHtml(confirmation.button)}
            </button>
          </form>
        </div>
      </section>
    ...
    res.send(layout(confirmation.title, body, "subscriptions"));
  } catch (error) {
    next(error);
  }
});
...
router.post("/subscriptions/:id/action", async (req, res, next) => {
  try {
    ...
    if (action === "suspend") data = { status: "SUSPENDED" };
      ...
      data = {
        status: subscription.complimentary ? "COMPLIMENTARY" : "ACTIVE",
    ...
    await writeAudit({
      ...
      entityType: "subscription",
      entityId: subscription.id,
      summary: `${action.replaceAll("_", " ")} for ${subscription.customer.name || subscription.customer.email}`,
    });
    res.redirect(
      `/admin/subscriptions/${encodeURIComponent(subscription.id)}?action=1`,
    );
  } catch (error) {
    next(error);
  }
  ...
  try {
    ...
    const body = `
      ...
      <section class="panel">
        ...
        <p class="muted">
          Each action requires confirmation before it is applied. Permanent deletion is only available once the licence is inactive.
        </p>

        <div class="actions">
          <a
            class="button secondary"
            href="/admin/licenses/${escapeHtml(licence.id)}/confirm/reactivate"
          >
            Reactivate
          </a>
          ...
          <a
            class="button warning"
            href="/admin/licenses/${escapeHtml(licence.id)}/confirm/suspend"
          >
            Suspend
          </a>
          ...
          <a
            class="button danger"
            href="/admin/licenses/${escapeHtml(licence.id)}/confirm/revoke"
          >
            Revoke
          </a>
          ...
          <a
            class="button secondary"
            href="/admin/licenses/${escapeHtml(licence.id)}/confirm/reset_activations"
          >
            Reset activations
          </a>
          ...
          <a
            class="button danger"
            href="/admin/licenses/${escapeHtml(licence.id)}/confirm/regenerate"
          >
            Regenerate key
          </a>
          ...
          ${
            canDelete
              ? `
                <a
                  class="button danger"
                  href="/admin/licenses/${escapeHtml(licence.id)}/confirm/delete"
                >
                  Delete permanently
                </a>
              `
...
router.post("/licenses/:id/update", async (req, res, next) => {
  try {
    ...
    await writeAudit({
      ...
      entityType: "license",
      entityId: req.params.id,
      summary: "Licence limits or expiry updated",
      metadata: { activationLimit, expiresAt: expiresRaw || null },
    });
    res.redirect(
      `/admin/licenses/${encodeURIComponent(req.params.id)}?saved=1`,
    );
  } catch (error) {
    next(error);
  }
...
router.get("/licenses/:id/confirm/:action", async (req, res, next) => {
  try {
    const licence = await prisma.license.findUnique({
      where: { id: req.params.id },
      include: {
    ...
    > = {
      reactivate: {
        ...
        message:
          "This licence will become active again and will be allowed to validate, subject to its linked subscription.",
        button: "Confirm reactivation",
        className: "primary",
      },
      suspend: {
        title: "Suspend licence?",
        message: "This licence will stop validating until it is reactivated.",
        button: "Confirm suspension",
        className: "warning",
      },
      revoke: {
        ...
        message:
          "This licence will immediately stop validating. It can only be used again if you explicitly reactivate it.",
        button: "Confirm revocation",
        className: "danger",
      },
      reset_activations: {
        title: "Reset all activations?",
        message:
          "All currently active installations for this licence will be deactivated. The licence itself will remain in its current status.",
        button: "Confirm activation reset",
        className: "warning",
      },
      regenerate: {
        title: "Regenerate licence key?",
        message:
          "The current licence key will stop working immediately. A replacement key will be generated and shown once.",
        button: "Confirm key regeneration",
        className: "danger",
      },
      delete: {
        title: "Permanently delete licence?",
        message:
          "This permanently deletes the licence, its activation history and its stored delivery record. This cannot be undone.",
        button: "Delete permanently",
        className: "danger",
      },
    };
    ...
    if (!confirmation) {
      return res
        ...
        .send(
          layout(
            "Licence",
            `<div class="alert error">Unknown licence action.</div>`,
            "licenses",
          ),
        );
    }
    ...
    if (
      action === "delete" &&
      !["SUSPENDED", "REVOKED", "EXPIRED"].includes(licence.status)
    ) {
      return res.status(400).send(
        layout(
          "Cannot delete licence",
          `<div class="alert error">This licence must be suspended, revoked or expired before it can be permanently deleted.</div>
           <a class="button secondary" href="/admin/licenses/${escapeHtml(licence.id)}">Back to licence</a>`,
          "licenses",
        ),
      );
    }
    ...
    const customerName =
      licence.customer?.name || licence.customer?.email || "Unassigned";
    const subscriptionText = licence.subscription
      ? `${licence.subscription.plan?.name || "Custom"} — ${statusLabel(licence.subscription.status)}`
      : "Legacy / unlinked";

    const formAction =
      action === "delete"
        ? `/admin/licenses/${escapeHtml(licence.id)}/delete`
        : `/admin/licenses/${escapeHtml(licence.id)}/action`;
    ...
    const body = `
      <section class="panel">
        ...
        <dl class="detail-list">
          <div>
            <dt>Licence</dt>
            <dd>${escapeHtml(licence.product.name)} · •••• ${escapeHtml(licence.keyLastFour)}</dd>
          </div>
          <div>
            <dt>Customer</dt>
            <dd>${escapeHtml(customerName)}</dd>
          ...
          <div>
            <dt>Subscription</dt>
            <dd>${escapeHtml(subscriptionText)}</dd>
          </div>
          <div>
            <dt>Current status</dt>
            <dd>${escapeHtml(licence.status)}</dd>
          </div>
          <div>
            <dt>Active activations</dt>
            <dd>${licence.activations.length} / ${licence.activationLimit}</dd>
          </div>
        </dl>
        ...
        <div class="actions">
          <a
            class="button secondary"
            href="/admin/licenses/${escapeHtml(licence.id)}"
          >
            Go back
          </a>
    ...
    res.send(layout(confirmation.title, body, "licenses"));
  } catch (error) {
    next(error);
  }
});
...
router.post("/licenses/:id/action", async (req, res, next) => {
  try {
    ...
    if (action === "regenerate") {
      ...
      await prisma.license.update({
        where: { id: licence.id },
        data: { keyHash, keyLastFour: rawKey.slice(-4) },
      });
      ...
      await writeAudit({
        ...
        entityType: "license",
        entityId: licence.id,
        summary: `Licence key regenerated for ${licence.customer?.name || licence.customer?.email || "unassigned customer"}`,
      });
    ...
    if (action === "reset_activations") {
      await prisma.activation.updateMany({
        where: { licenseId: licence.id, deactivatedAt: null },
        data: { deactivatedAt: new Date(), lastCheckedAt: new Date() },
      });
    ...
    await writeAudit({
      ...
      entityType: "license",
      entityId: licence.id,
      summary: `${action.replaceAll("_", " ")} on licence •••• ${licence.keyLastFour}`,
    });
