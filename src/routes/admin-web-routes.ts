...
router.post("/plans", async (req, res, next) => {
  try {
    ...
    await prisma.plan.create({ data: { productId: String(req.body.product_id), name: String(req.body.name).trim(), slug: String(req.body.slug).trim().toLowerCase().replace(/[^a-z0-9-]/g,"-"), priceMinor: price ? Math.round(Number(price) * 100) : null, billingInterval: String(req.body.billing_interval ?? "").trim() || null, stripeProductId: String(req.body.stripe_product_id ?? "").trim() || null, stripePriceId: String(req.body.stripe_price_id ?? "").trim() || null, entitlements: { create: entitlements } } });
...
router.post("/plans/:id", async (req, res, next) => {
  try {
    ...
    const price = String(req.body.price ?? "").trim();
    await prisma.$transaction(async tx => {
      await tx.plan.update({ where: { id: req.params.id }, data: { name: String(req.body.name ?? "").trim(), description: String(req.body.description ?? "").trim() || null, priceMinor: price ? Math.round(Number(price) * 100) : null, billingInterval: String(req.body.billing_interval ?? "").trim() || null, stripeProductId: String(req.body.stripe_product_id ?? "").trim() || null, stripePriceId: String(req.body.stripe_price_id ?? "").trim() || null, active: req.body.active === "1" } });
    ...
    const body = `
      <section class="panel">
        ...
        <div class="actions">
          <form
            method="get"
            action="/admin/subscriptions/${escapeHtml(subscription.id)}"
          >
            <button class="button secondary" type="submit">
              Go back
            </button>
  ...
  try {
    ...
    > = {
      ...
      suspend: {
        ...
        message:
          "This licence will stop validating until it is reactivated.",
        button: "Confirm suspension",
        className: "warning",
      },
      revoke: {
    ...
    const body = `
      <section class="panel">
        ...
        <div class="actions">
          <form
            method="get"
            action="/admin/licenses/${escapeHtml(licence.id)}"
          >
            <button class="button secondary" type="submit">
              Go back
            </button>
