...
customerPortalRouter.get("/", async (req, res, next) => {
  try {
    ...
    if (!customerId) {
      const msg =
        req.query.sent === "1"
          ? `<div class="alert success">If that email belongs to an RWExec customer, a secure sign-in link has been sent.</div>`
          : "";
      return res.send(
        shell(
          req,
          "Customer account",
          `<section class="panel portal-login"><img class="portal-logo" src="${portalPath(req, "/assets/rwexec-logo.png")}" alt="RWExec"><h1>Customer account</h1><p class="muted">Enter your RWExec account email and we’ll send a secure sign-in link.</p>${msg}<form class="form-grid" method="post" action="${portalPath(req, "/request-link")}"><label>Email address<input type="email" name="email" required></label><button class="button primary" type="submit">Email sign-in link</button></form>${customerEmailConfigured() ? "" : `<div class="alert error" style="margin-top:16px">Customer email delivery is not configured yet. Contact RWExec support for access.</div>`}</section>`,
        ),
      );
    }
    ...
    const subs = customer.subscriptions
      .map(
        (s) =>
          `<tr><td>${escapeHtml(s.product.name)}</td><td>${escapeHtml(s.plan?.name || "Custom")}</td><td><span class="status ${s.status.toLowerCase()}">${escapeHtml(s.status.replaceAll("_", " "))}</span></td><td>${s.currentPeriodEnd ? escapeHtml(s.currentPeriodEnd.toISOString().slice(0, 10)) : "Never"}</td></tr>`,
      )
      .join("");
...
customerPortalRouter.post("/request-link", async (req, res, next) => {
  try {
    ...
    res.redirect(`${portalPath(req)}?sent=1`);
  } catch (error) {
    next(error);
  }
});
      ...
      return res
        ...
        .send(
          shell(
            req,
            "Link expired",
            `<section class="panel portal-login"><h1>That sign-in link is no longer valid</h1><p class="muted">Request a new secure link from the customer account page.</p><a class="button primary" href="${portalPath(req)}">Request a new link</a></section>`,
          ),
        );
    }
      ...
      return res
        ...
        .send(
          shell(
            req,
            "Licence unavailable",
            `<section class="panel"><h1>Licence key unavailable</h1><p class="muted">This key has already been collected or its delivery window has expired. Contact RWExec support if you need the key regenerated.</p><a class="button secondary" href="${accountUrl}">Back to account</a></section>`,
          ),
        );
    }
