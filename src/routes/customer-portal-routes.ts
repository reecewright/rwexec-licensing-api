  ...
  try {
    ...
    const customer = await prisma.customer.findUnique({
      ...
      include: {
        ...
        licenses: { orderBy: { createdAt: "desc" }, include: { product: true, delivery: true, activations: { where: { deactivatedAt: null } } } },
      },
    });
...
customerPortalRouter.get("/verify", async (req, res, next) => {
  try {
    ...
    if (!customerId) {
      return res.status(400).send(shell(req, "Link expired", `<section class="panel portal-login"><h1>That sign-in link is no longer valid</h1><p class="muted">Request a new secure link from the customer account page.</p><a class="button primary" href="${portalPath(req)}">Request a new link</a></section>`));
    }
...
customerPortalRouter.post("/licenses/:id/reveal", async (req, res, next) => {
  try {
    ...
    if (!licence) {
      return res.status(404).send(shell(req, "Licence not found", `<section class="panel">Licence not found.</section>`));
    }

    const rawKey = await claimLicenceDelivery(licence.id, customerId);
    if (!rawKey) {
      return res.status(409).send(shell(req, "Licence unavailable", `<section class="panel"><h1>Licence key unavailable</h1><p class="muted">This key has already been collected or its delivery window has expired. Contact RWExec support if you need the key regenerated.</p><a class="button secondary" href="${accountUrl}">Back to account</a></section>`));
    }

    res.send(shell(req, "Your licence key", `<section class="panel portal-login"><h1>${escapeHtml(licence.product.name)} licence</h1><div class="alert success"><strong>Copy this key now.</strong> For security it will not be shown again.</div><div class="secret">${escapeHtml(rawKey)}</div><p class="muted">If you lose this key later, RWExec can regenerate it. Regenerating invalidates the previous key.</p><a class="button secondary" href="${accountUrl}">Back to account</a></section>`));
  } catch (error) {
    next(error);
  }
});
